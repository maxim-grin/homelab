...
| Vault data             | `/opt/vault/data`, `vault-01` | seeded once above | **Yes.** Rebuild the LXC, re-init, re-unseal, then `-e vault_seed=true -e vault_token=...` replays `vault_kv` from `secret.yaml` |
