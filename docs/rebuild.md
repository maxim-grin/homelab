...
the SSD; Vault's data lives on the LXC's own disk, also on the SSD. None of
it is backed up.
...
| Vault data             | `/opt/vault/data`, `vault-01` | seeded once above | **Yes.** Rebuild the LXC, re-init, re-unseal, then `-e vault_seed=true -e vault_token=...` replays `vault_kv` from `secret.yaml`. `vault-02`'s raft snapshots land daily in `/srv/nfs/backups` on `nfs-01`, 14 kept — same SSD |
...
9. **Build `vault-02`** — the VM replacing the `vault-01` LXC. Install
   the collections it needs, then install Vault on the VM:
...
11. **`kubectl apply -f argocd/base/projects.yaml`** — the AppProject. Nothing
...
12. **`kubectl apply -f argocd/environments/dev/applications/app-of-apps.yaml`**
...
14. **Configure Vault's Kubernetes auth** — only after `argocd-config` shows
    ...
    `system:serviceaccount:argocd:argocd-repo-server` — the path AVP actually
    uses to authenticate, never the root token. The task that posts the
...
`vault operator unseal`, step 9 above):
